# Скрипт выполняет поиск файлов чертежей на жестком диске.
# Номера чертежей задаются файлом в формате txt. Каждый номер должен располагаться на отдельной строке.

import os
import re
import subprocess

def input_drawings_numbers():
    # Эта функция возвращает лист из номеров чертежей которые мы хотим искать, после ввода имени файла с названиями.
    right_input = False
...
def input_files_location():
    # Эта функция возвращает путь к папке в которой, мы указали, храняться файлы чертежей.
    result = False
    while not result:
...
def input_copy_location():
    # Эта функция возвращает(при необходимости создает) путь к папке куда найденые файлы будут скопированы
...
def create_files_list(files_location):
    # Эта функция возвращает лист названий всех файлов в папке которую мы указали.
    try:
...
def rename_draw_num(names_data):
    # Эта функция возвращает лист с номерами чертехей,
    # подготовленных для поиска с модулем re
    re_names_data = []
...
def find_copy_files(files_location, copy_loc, files_list, re_names_data):
    # Эта функция осуществляет сравнение
    # стандратного выражения сформированого для номера чертежа
    # и названия файлов в папке где файлы расположены.
    # при совпадении стандартного выражения и названия файла
    # файл копируется в папку которую мы указали.
